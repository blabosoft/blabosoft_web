...
It's a common situation when you have to process items stored somewhere using background jobs. I think it is more common when you can't (don't want to) ensure that only one job instance access the items at the same time.
...
However it is possible that you don't want to start using a new componenet to have simple queue functionality, but you already use a relational database like PostgreSQL. I show you a very simple implemenation of queue operations (push, pop) using relational database. Applying this method your jobs can pop items atomically from the queue so having multiple job instances at the same shouldn't be a problem.
...
- `errors` : If the processing fails for any reasons, you can store error log here
- `payload` : You can store any data in textual format, for example as a `JSON`

The `created_at` and `processing_started_at` columns are in the heart of the queue operations, I will describe it later in this article. As you can see there is an index for these columns to ensure the proper performance of the queue operations.
...
The `update` command can be used with condition and also can return data. __In this way we can achieve the consistent pop operation implementing an atomic 'test-and-set' in the database.__
...
In the above example we use the `processing_started_at` column for checking whether the processing of an item has been started or not and we atomically set the value of first unprocessed item. 
...
There are many good solutions for handling such situations, but I don't cover it in this article.
...
<!-- I created an implementation for SQL Server as well, you can download it as an [SQL file here](/assets/posts/2021-09-08/queue-sql-server.txt){:target="_blank"}. -->